        ...
        if (points != "none") {
            ...
            gg <- gg +
                scale_colour_manual(name = "Type",
                                    values = c("black", "orangered",
                                               "SpringGreen3", "SteelBlue3"))
            gg <- gg + scale_shape_manual(name = "Type",
                                          values = c(21, 21, 22, 24))
            gg <- gg +
                scale_fill_manual(name = "Type",
                                  values = c("black", "gold",
                                             "SpringGreen1", "SteemBlue1"))
        }
...
autoplot.confintCheck.poisGP <- function(object, ...) {
    ...
    gg <- gg + geom_line(data = object$negLogLikC,
                         mapping = aes_string(x = "Par", y = "Value",
                                              group = "Name"))
    ...
    gg <- gg + geom_hline(data = object$ci,
                          mapping = aes_string(yintercept = "NegLogLik",
                                               group = "Name",
                              linetype = "Level", colour = "Level"))
...
##' @return A ggplot graphics object.
##'
##' @seealso \code{\link{RL.poisGP}} and the example with \code{check = TRUE}
##' in the \bold{Examples} section therein.
##' \code{\link{autoplot.confintCheck.poisGP}} is a similar method to
...
autoplot.RLCheck.poisGP <- function(object, ...) {
    ...
    gg <- ggplot()
    gg <- gg + geom_line(data = object$negLogLikC,
                         mapping = aes_string(x = "rho", y = "Value",
                                              group = "Period"))
