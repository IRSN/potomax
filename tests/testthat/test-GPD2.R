...
f <- function(theta) {
    dGPD2(x, scale = theta[1], shape = theta[2], log = TRUE)
}
...
test_that(desc = "Gradient of the GPD2 log-density",
...
test_that(desc = "Gradient of the GPD2 quantile",
            expect_lt(max(abs(attr(fval, "gradient") - Jnum)), 1e-6))

test_that(desc = "Hessian of the GPD2 quantile",
            expect_lt(max(abs(drop(attr(fval, "hessian")) - Hnum)),
                      1e-4))
