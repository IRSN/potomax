...
The **potomax** package has been financed by IRSN/Behrig and was
designed in response to some of the Behrig's practical needs in
...
select the exceedances $Y_i> v$ over a higher threshold $v > u$, the
same assumptions hold for a different rate and GP scale.
...
case where no event occur in the interval, the r.v. $M$ follows a
Gumbel distribution. Moreover if we consider a partition of the time
axis into time intervals or time blocks $b =1$, $2$, $\dots$ with the
same duration $w$, we get i.i.d. random variables $M_b$.
...
**evd** [@PACKevd], **ismev** [@PACKismev], **POT** [@PACKPOT].  See
also Northrop et al [@NorthropEtAl_NSExtremes] for a presentation of
the two frameworks.
...
The main class is `"PoisGP"`. The creator `data` which can be a simple
numeric vector or more complex data structures such as `"Rendata"`
from **Renext** or `"potData"` from **potomax**. We begin by using the
`Garonne` object from **Renext**. This object contains observations
for the discharge rate of the river Garonne (France) in cumecs. It
embeds both "systematic" observations and "historical" observations.
...
from the default parameterisation to the `"NHPP"`
one. Profile-likelihood can also be used to infer on the return levels
...
intervals on parameters are in **potomax** computed by using
...
bound of the interval by minimising $\eta$ under
the same constraint.

Remind that the classical method consists in computing the profile
log-likelihood function $\ell_{\text{prof}}(\eta)$ and in solving
numerically the equation $\ell_{\text{prof}}(\eta) =
\ell_{\text{max}} - \delta$.  Provided that the log-likelihood
function is well-shaped, two solutions exist: one on each side of the
ML estimate $\widehat{\eta}$. These are the lower and upper bounds of
the confidence interval $(\eta_{\text{L}}, \, \eta_{\text{U}})$.  Each
evaluation of the profile log-likelihood requires an optimisation on
$2$ parameters, and moreover we have to solve the two equations by
using either a zero-finding method or a search on a grid of candidate
values for $\eta$.

### Checking the results

The constrained optimisation method is appealing for its generality.
However, the optimisation can fail which can be quite difficult to
diagnose. The classical approach solving equations for the profile
log-likelihood may also fail, yet the results can be checked by
plotting the the negative profile-likelihood
$-\ell_{\text{prof}}(\eta)$ against the parameter $\eta$, which should
lead to a smooth curve with a minimum at the ML estimate
$\widehat{\eta}$. By adding the horizontal line corresponding to the
negative log-likelihood ordinate $-(\ell_{\max} - \delta)$, the confidence
bounds $\eta_{\text{L}}$ and $\eta_{\text{U}}$ should be the abscissas
...
In the simplest use, the formal argument `data` can be a numeric
vector giving the marks $Y_i$. Then one must also use the formal
argument `effDuration` to give the so-called *effective* duration.
The process may have been observed on several intervals, and the
duration must take into account possible gaps in the data.

`MAX.data` and `OTS.data` censoring by number or by value. In both
cases one must give a list, which must be completed by a numeric
vector of durations, `MAX.effDuration` or `OTS.effDuration`. Each
element of the "data" list corresponds to an interval of time and
contains either the largest observations (for `MAX.data`) or all the
...
also give a numeric vector of thresholds.

`MAX.data` can be used for the classical block maxima and the
$r$-largest observations. 
...
the years for block maxima or $r$-largest observations. The same
element names should then be used for the data lists, the vector of
durations and the vector thresholds in the "OTS" case.
...
order to stick to the Poisson-GP parameterisation.
...
As written by Coles, *the plots again indicate a fundamental lack of fit for the
model”. Note that the smallest observations indicate a neat concave curvature, while
the largest ones seem to better conform to an exponential tail. We can 
describe this by censoring the  the MAX data, on the basis of the Poisson-GP model.
We chose a threshold of $110$ cm corresponding to [”very intense” aqua alta](https://en.wikipedia.org/wiki/Acqua_alta).
...
shape (green diamond) is now used for the corresponding blocks. Note
...
the marks $Y_i$, but also block maxima by using suitable plotting
positions. These positions are designed to get points around a
straight line in the exponential-Gumbel case $\xi = 0$, both for the
marks and the maxima.
...
arguments, as used in most R packages. They differ from the Weibull
positions used in **Renext**. As a consequence, the largest
...
In the general framework considered in **potomax**, the concept number
of observations is unclear, hence so are AIC and BIC. The concept of
number of observation makes sense when the observations have the same
marginal distribution and are independent or correspond to a form of
stationarity. But in the framework of **potomax**, one observation for
a long time interval may be more informative than several observations
